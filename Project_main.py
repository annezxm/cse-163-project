...
def preferred_foot_plot(dataset):
    ...
    '''
    df = dataset.groupby('preferred_foot')['year'].count()
    foot_list = [df[0], df[1]]
    foot_labels = ['left', 'right']
    plt.pie(foot_list, labels=foot_labels, autopct='%.0f%%')
...
def wage_predict(df, df_22):
    ...
    features = data.loc[:, ['overall', 'potential', 'value_eur']]
    ...
    features_22 = df_22.loc[:, ['overall', 'potential', 'value_eur']]
...
def main():
    ...
    data_22_top100 = data_22.nlargest(100, 'overall')
    ...
    nationality_histogram(data_22, 'most_player_22.png',
                          'Top 10 countries with most players')
    nationality_histogram(data_22_top100, 'most_player_top100.png',
                          'Top 10 countries with Top 100 players')
